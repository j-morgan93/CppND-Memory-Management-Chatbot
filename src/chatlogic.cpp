                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // _edges.emplace_back(std::move(edge)); apparently not use??? Why is this still in here then?
  ...
  GraphNode *rootNode = nullptr;
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); // assign current node to root from **knowledge.udacity.com/questions/129814
            }
            else
            {
    ...
    // add chatbot to graph root node, re-do of the handling earlier
  ChatBot chatty("../images/chatbot.png");
  SetChatbotHandle(&chatty); //have to set chatbothandle
